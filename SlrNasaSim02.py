# SlrNasaSim02.py
# Last Update 2025-11-18T18:02:00Z
...
class ScrollWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Satellite Laser Ranging Simulator - NASA Geodesy")
        ...
        ax_range.set_title('Laser Ranging Photon Returns from Satellite', color='white', fontsize=14)
        ...
        # Live histogram — now labeled in ns
        ...
        ax_live.set_xlabel('TOF Residual (ns)')          # ← NEW: ns label
        ax_live.set_ylabel('Count')
        ax_live.set_title('Live Return Pulse (last 1 s)', color='white')

        # Cumulative histogram — now labeled in ns
        ...
        ax_cum.set_xlabel('TOF Residual (ns)')           # ← NEW: ns label
        ...
        fig.suptitle('Tracking Satellite: LAGEOS-1 from Station: NASA GGAO', fontsize=16, fontweight='bold', color='#58a6ff')
        canvas.draw_idle()
